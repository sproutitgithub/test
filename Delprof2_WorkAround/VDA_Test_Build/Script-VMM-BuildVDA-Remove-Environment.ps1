...
#region Stop and delete VM 

Write-Host "Delete Test Build" -ForegroundColor Yellow

Invoke-Command -ComputerName vmmcluster01 -Credential $MyCredential1 {
$InsideVM = $using:outsideVDANAMETRIMMED
Write-Verbose "VM to be stopped and deleted is $($InsideVM)" -Verbose
Start-Sleep 4
$VM = Get-SCVirtualMachine -VMMServer vmmcluster01 | ? {$_.Name -match $InsideVM}
IF (($VM))
{
    Write-Host "Attempting to stop $($VM.Name)" -ForegroundColor Yellow
    Stop-SCVirtualMachine -VM $VM -RunAsynchronously
        do {
    $VM = Get-SCVirtualMachine -VMMServer vmmcluster01 -Name $VM.Name
    Write-Host "Waiting for $($VM.Name) to power off" -ForegroundColor Green
    Start-Sleep 1
        }until($VM.Status -eq 'Poweroff')

    Write-Host "Attempting to remove VM $($VM.Name)" -ForegroundColor Yellow
    Start-Sleep 1
    Get-SCVirtualMachine -VMMServer vmmcluster01 -Name $VM.Name | Remove-SCVirtualMachine -RunAsynchronously
    do {
    Write-Host "Waiting removal of VM $($VM.Name)" -ForegroundColor Yellow
    $VM = Get-SCVirtualMachine -VMMServer vmmcluster01 -Name $VM.Name    
    Start-Sleep 1
    }until(!($VM.Name))
}
    ELSE
    {
        Write-Verbose "No VM Identified in environment" -Verbose
        Start-Sleep 1
    }
}
