...
#region Write Results and update LIVE if successful 

    Write-Verbose "Value of 1 detected so test build is good" -Verbose
    Start-Sleep 1
   ...
   $Results =  [pscustomobject]@{
    ...
    Write-Verbose "Moving TEST task sequence TS.XML to Live"
    copy E:\DeploymentShare\Control\500\ts.xml -Destination E:\DeploymentShare\Control\300 -Force
    $HASHT = Get-FileHash E:\DeploymentShare\Control\500\ts.xml
    $HASHL = Get-FileHash E:\DeploymentShare\Control\300\ts.xml
    IF ($HASHT.Hash -match $HASHL.Hash)
    {
        Write-Verbose "Hashes match" -Verbose
    }
